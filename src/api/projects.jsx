import rtu from "../assets/rtu.webp";
import abacus from "../assets/abacus.webp";
import encryp from "../assets/encrypted.webp";
import skycast from "../assets/skycast.webp";
import bitvortex from "../assets/bitvortex.png";
import tinyurl from "../assets/tinyurl.png";
import crudify from "../assets/crud.png";
import luxebay from "../assets/luxebay.png";
import recruit from "../assets/recruit.png";
import productf from "../assets/productf.png";

export const projects = [
  {
    imgsrc: rtu,
    tech: "React, Tailwind, Firebase",
    title: "RTU POS",
    description:
      "RTU POS: Seamlessly integrated with the RTU Apparel eCommerce site, it’s your one-stop solution for easy and efficient point-of-sale transactions.",
    src: "https://github.com/sIUzyy/p-rtupos",
    url: "https://rtu-pos.web.app",
    web: "rtu-pos.web.app",
    date: "April 2023",
    email: "rtuadmin2023@gmail.com",
    pass: "123456",
  },

  {
    imgsrc: rtu,
    tech: "React, Tailwind, Firebase",
    title: "RTU Apparel",
    description:
      "RTU Apparel: Your online fashion destination for trendy styles and effortless shopping.",
    src: "https://github.com/sIUzyy/p-rtuapparel",
    url: "https://rtu-apparel.web.app",
    web: "rtu-apparel.web.app",
    date: "March 2023",
    email: "siuzysaur@gmail.com",
    pass: "123456",
  },

  {
    imgsrc: productf,
    tech: "React & Tailwind",
    title: "Product finder",
    description:
      "Discover, Compare, Shop: Navigate Every Category, Find Every Price on ProductFinder!",
    src: "https://github.com/sIUzyy/p-productfinder",
    url: "https://product-finderr.web.app/",
    web: "product-finderr.web.app",
    date: "August 2023",
    email: "",
    pass: "",
  },

  {
    imgsrc: recruit,
    tech: "React & Tailwind",
    title: "RecruitEase",
    description:
      "Unlock Your Hiring Potential with RecruitEase: Your Online Recruitment Solution",
    src: "https://github.com/sIUzyy/p-recruit",
    url: "https://recruiteasee.web.app/",
    web: "recruiteasee.web.app",
    date: "July 2023",
    email: "",
    pass: "",
  },

  {
    imgsrc: luxebay,
    tech: "React, Redux, Tailwind",
    title: "Luxebay",
    description: "Unlock Luxury, Discover LuxeBay",
    src: "https://github.com/sIUzyy/p-luxebay",
    url: "https://luxebay-ecommerce.web.app/",
    web: "luxebay.web.app",
    date: "June 2023",
    email: "siuzysaur@gmail.com",
    pass: "123456",
  },

  {
    imgsrc: crudify,
    tech: "React, Redux, Tailwind",
    title: "CRUDify",
    description: "Empowering Simplicity: CRUDify - Seamlessly Manage Your Data",
    src: "https://github.com/sIUzyy/p-crudify",
    url: "https://crudifyyy.web.app/",
    web: "crudifyyy.web.app",
    date: "June 2023",
  },

  {
    imgsrc: tinyurl,
    tech: "React, Tailwind, Shrtco.de API ",
    title: "TinyURLs",
    description:
      "Elevate Your Links: Simplify, Share, and Track with TinyURLs!",
    src: "https://github.com/sIUzyy/p-tinyurls",
    url: "https://tinyurlss.web.app/",
    web: "tinyurlss.web.app",
    date: "June 2023",
  },

  {
    imgsrc: bitvortex,
    tech: "React, Tailwind, Coingecko API",
    title: "Bitvortex",
    description:
      "BitVortex: Unleash the Power of Cryptocurrency with Confidence. Seamlessly integrate our advanced API web platform to access real-time data.",
    src: "https://github.com/sIUzyy/p-bitvortex",
    url: "https://bitvortex-crypto.web.app/",
    web: "bitvortex.web.app",
    date: "May 2023",
  },

  {
    imgsrc: skycast,
    tech: "React, Tailwind, Openweather API ",
    title: "SkyCast",
    description:
      "Skycast: Your go-to weather API for real-time and accurate forecasts worldwide. Plan, stay informed, and make confident decisions with ease.",
    src: "https://github.com/sIUzyy/p-skycast",
    url: "https://skycast-weatherapi.web.app/",
    web: "skycast.web.app",
    date: "May 2023",
  },

  {
    imgsrc: encryp,
    tech: "React & Tailwind",
    title: "EncryptedX",
    description:
      "EncryptedX: Unveiling encryption’s secrets. Explore cryptography, privacy, and secure communication in a concise, informative website. Enhance your digital security now.",
    src: "https://github.com/sIUzyy/p-encryptedX",
    url: "https://encryptedx.netlify.app",
    web: "encryptedX.netlify.app",
    date: "May 2023",
    email: "",
    pass: "",
  },

  {
    imgsrc: abacus,
    tech: "Html, Css, Javascript",
    title: "Abacaus Academy",
    description:
      "Abacus Academy: Empowering learners through immersive boot camps. Unlock your potential and master in-demand skills.",
    src: "https://github.com/sIUzyy/p-abacus-academy",
    url: "https://abacusacademy-f06cc.web.app",
    web: "abacusacademy.web.app",
    date: "Oct 2022",
    email: "",
    pass: "",
  },
];
